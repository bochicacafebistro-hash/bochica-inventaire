...
- [x] **Cacher les taux/salaires côté base pour le rôle Employé** — ✅ **Fait (v3.43.0, 8 juin 2026)**. Rémunération déplacée dans `/employeesComp/{empId}` (admin only) ; fusion transparente dans `employees` en mémoire côté admin via `_applyEmployeeComp()` (aucune lecture de taux à modifier) ; migration auto idempotente `migrateEmployeeComp()` qui retire les champs de `/employees`. ⚠ Déploiement : republier `firestore.rules` **puis** se connecter une fois en admin pour déclencher la migration.
