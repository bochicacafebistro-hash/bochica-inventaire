...
> 🗺️ **Feuille de route de la v2** (ce qui reste à faire, étapes 5 à 7, et la structure du projet expliquée simplement) : document « claude/feuille-de-route-v2.md » du projet Claude « bochica-inventaire ».
>
> 📌 **Voir `TODO.md`** à la racine du repo pour la liste vivante des améliorations à venir (sécurité, food cost, vue mobile, tests, etc.).

> 🔒 **26 septembre 2026 — Verrouillage de la paie appliqué par la base** — `firestore.rules` (`/payroll`) : quand une semaine est verrouillée (`locked == true`), la base refuse toute modification de ses heures, pourboires, ventes, bonus, dérogations, extras (seuls les champs du verrou — `locked`, `lockedAt`, `lockedAmount`, `expenseId`, `lockedBy` — et la méta `updatedAt`/`weekId`/`weekStart` peuvent changer, pour déverrouiller) et sa suppression, pour TOUS les comptes (admin compris) et les deux apps. Avant, le verrou n'était qu'un blocage d'écran (contournable par une erreur de code : incident du 25 sept.). Messages clairs dans la v2 : Salaires (« semaine verrouillée ») et Pointage FR/ES (« semaine de paie verrouillée, avise l'admin »). ⚠ **À publier dans la console Firebase** pour être actif. Tests de règles avec l'émulateur impossibles ici (téléchargement bloqué) : relu à la main ; vérifiable avec le simulateur de règles de la console.
>
> 🚨 **26 septembre 2026 — INCIDENT paie (v2) : heures pointées écrasées, semaines 37, 38 et 39** — Le 25 sept. vers 23 h 36, en changeant de semaine dans Salaires & Pourboires (v2), le remplissage automatique a tourné sur les données **de la semaine précédente** (useDocument gardait l'ancien document pendant le changement d'id) : il a cru que personne n'avait pointé et a écrit l'horaire prévu (`autoFilled` + `autoFilledNoStart`, « PRÉSENCE ? ») par-dessus les heures réellement pointées. Bilan lu dans Firestore (lecture seule) : W37 = 23 quarts écrasés, W38 = 27, W39 = 13 (tous `autoFilledAt` = 2026-09-25 23 h) ; les semaines ≤ W36 sont intactes. La v1 affiche les mêmes données (même base). **Pas de récupération à un moment donné (PITR) activée** : lecture `readTime` refusée au-delà d'une heure → les heures d'origine ne sont plus dans Firestore. **Corrections (en ligne le 26 sept. ~9 h 36)** : `useDocument` ne renvoie plus jamais les données d'un autre document (repasse en chargement quand l'id change) ; la v2 ne remplit plus **jamais** automatiquement (bouton « Remplir depuis l'horaire » sur clic, écrit seulement la sortie quand une entrée existe) ; le Pointage n'écrit plus que le champ pointé (l'autre est conservé par la fusion Firestore). La v1 n'avait pas ce défaut (elle attend que le document soit chargé). **À faire** : activer PITR + sauvegardes planifiées dans Google Cloud ; ressaisir les quarts « PRÉSENCE ? » des semaines 37 à 39 à partir des PDF de paie / de l'équipe.
>
> ⚠️ **26 septembre 2026 — v2 : étape 5 terminée — migration complète** — Les 3 derniers morceaux sont en React ; `registry.ts` n'a plus aucun module `legacy`. Seule fonction encore uniquement dans la v1 : l'import des rapports mensuels (PDF Cluster). **(1) PDF de paie** (`app/src/modules/paie/payrollPdf.ts`, menu ⋮ de Salaires) : 1 semaine et 2 semaines (semaine affichée + précédente), même mise en page que la v1 (Letter paysage, papier crème). Les montants viennent maintenant du **même calcul que l'écran** (`computePayroll`) : la v1 recalculait à part — son PDF 1 semaine ignorait les dérogations de pool et « sans pourboire », son PDF 2 semaines comptait aussi les employés retirés de la semaine. **Corrigé** : un salarié sans pointage était omis des PDF (alors qu'il est payé) ; il est maintenant imprimé. Filtre d'impression sinon identique (non archivé, taux > 0, heures > 0). **(2) Simulation paie** (`app/src/modules/simulation/`, collection `payrollSimulations`, même format : `baseline` figé + `simulation` modifiable, quarts par jour de semaine 0..6, quarts coupés) : liste avec écart vs réel, éditeur (paramètres pourboires / part cuisine / ratio, grille 7 jours avec jours fermés, ajouter/modifier/**déplacer** un quart par choix du jour, nom/taux/section en ligne, monter/descendre, employé fictif, heures de service, jours ouverts, couverture en carte de chaleur, comparaison réel ↔ simulation), créer (choix de la semaine à copier : dernière, cette semaine, prochaine, dans 2 semaines), renommer, copier l'horaire de cette semaine, dupliquer, réinitialiser, supprimer, images PNG équipe et admin. **Test de parité** avec `computeSimScenario` / `simEffectiveOpenDays` / `countSimCoverageAtHour` de la v1 sur 200 scénarios : identiques. **(3) Tableau de bord admin** = page d'accueil de l'admin (`accueil/AdminDashboard.tsx` + `dashboard.logic.ts`) : bannières (congés en attente, pointages à vérifier, frais fixes à créer), « Aujourd'hui » (qui est en quart par section, événements de la semaine, tâches dues, **ratio salaires/ventes de la semaine rebranché** sur le calcul de Salaires & Pourboires — vide dans la v1 depuis la v3.76), revenus / dépenses / profit du mois vs mois précédent avec courbe 30 jours, marge moyenne du menu, TPS/TVQ du trimestre (+ « Remis ✓ » si la remise est enregistrée), prochains événements (60 j), stock critique, tâches en retard, plus grosses dépenses. **Corrigé** : la v1 prenait la date UTC pour « aujourd'hui » (après 20 h l'été, le tableau de bord affichait déjà le lendemain) et ses courbes 30 jours étaient décalées d'un jour. Le chef a maintenant une page d'accueil de raccourcis. Refactor : `CoverageGrid` générique (horaire + simulation), briques PNG exportées de `schedulePng.ts`, `OpenDaysModal` avec texte d'aide optionnel. Frais fixes : toujours le bouton manuel dans Dépenses (automatique à la bascule, étape 6). 155 tests.
>
> ⚠️ **25 septembre 2026 — v2 : étape 5.3 — « Salaires & Pourboires » migré (sauf PDF)** — `app/src/modules/paie/` (admin). Même format que la v1 : `payroll/{AAAA-Wnn}` (actualShifts, tipsByDay, netByDay, bonusByEmp, sectionOverrides, manualEmployees, hiddenEmps, locked/lockedAmount/expenseId), `settings/payroll` (tipShares, defaultServiceHours, une plage ou plusieurs), ordre partagé avec l'horaire (`settings/schedule.weekOrder`). **Un seul calcul** (`computePayroll` dans `paie.logic.ts`) : heures réelles, salaire (taux daté par jour ; salarié = heures fixes × taux du lundi), pourboires **partagés jour par jour** au prorata des heures faites pendant les heures de service (pool Cuisine / pool Service + Autre ; dérogation de la semaine > « sans pourboire » > section), bonus, coût réel vs prévu, ratio salaires/ventes (vert ≤ cible, orange ≤ cible + 8 pts), rentabilité (% des ventes nécessaires atteint). **Test de parité** avec `_computePayrollWeekData` de la v1 sur 150 semaines générées (service coupé, quarts de nuit, dérogations, salariés, hausse de taux en milieu de semaine) : montants identiques. Mêmes règles pour les **alertes** (sortie manquante, sortie remplie automatiquement, présence à vérifier, quart > 14 h, prévu sans pointage) et le **remplissage automatique** 1 h après la fin prévue. Écran : saisie par jour des pourboires et ventes nettes, grille des heures (absent, incomplet, congé, prévu non pointé, à valider), modifier/déplacer/marquer absent/réinitialiser un jour, pool de pourboires par employé pour la semaine, bonus, extras de la semaine, retirer/réordonner, heures de service, répartition %, effacer la semaine (taper EFFACER), verrouiller (crée la dépense « Salaires sem. N ») / déverrouiller (la supprime), récapitulatif des pools, 2 graphiques à un seul axe avec vue tableau. **Corrigé par rapport à la v1** : le montant de la dépense au verrouillage comptait aussi les employés archivés ou retirés de la semaine (un salarié archivé ajoutait un salaire complet) → maintenant salaires + bonus des employés affichés ; pourboires, ventes et bonus ne sont plus modifiables quand la semaine est verrouillée. **À venir (5.3b)** : PDF de paie 1 et 2 semaines — le PDF 2 semaines de la v1 ignore « sans pourboire » et inclut les employés retirés, la v2 utilisera le même calcul que l'écran. Nouveau : `useDataActions().replace` (écriture sans fusion), `loadV1Multi` (tests de parité sur plusieurs fichiers v1). 139 tests.
>
> ⚠️ **25 septembre 2026 — v2 : étape 5.2 — « Pointage » migré** — `app/src/modules/pointage/` (FR/ES, les 3 rôles). Même format que la v1 : les pointages vont dans `payroll/{AAAA-Wnn}.actualShifts[employé][jour]` (`{start, end}`, les marques `autoFilled*` / `markedAbsent*` retirées à chaque pointage, comme `updateActualShift`), donc Salaires & Pourboires (encore en v1) se remplit tout seul et les deux apps peuvent pointer en même temps. Règles identiques : NIP à 4 chiffres (même clavier que la demande de congé, clavier physique accepté), deux gros boutons ENTRÉE / SORTIE toujours visibles (re-pointer remplace), la date est recalculée au moment exact du pointage, **quart de nuit** : une SORTIE avant 10 h sans entrée aujourd'hui ferme le quart resté ouvert la veille si la durée est ≤ 12 h — y compris quand la veille est dans la semaine précédente (nuit dimanche → lundi). Écritures limitées aux champs permis aux employés par `firestore.rules` (`actualShifts`, `weekId`, `weekStart`, `updatedAt`). Nouveautés : quart prévu du jour affiché (« Prévu aujourd'hui : 16:00–23:00 »), retour automatique au clavier après 30 s sans action sur l'écran d'un employé (tablette partagée), erreur de NIP affichée sous les points au lieu d'un écran séparé. Tests : identifiant de semaine de paie **identique à la v1 sur 3 ans** (test de parité), règles du quart de nuit. Restent en v1 : Salaires & Pourboires, Simulation paie, Tableau de bord admin.
>
> ⚠️ **25 septembre 2026 — v2 : étape 5.1 — « Employés & Horaires » migré** — `app/src/modules/equipe/EmployesPage.tsx` (+ `horaire.logic.ts`, composants dans `components/`). Mêmes données que la v1 : fiches `employees` (quarts `shifts.{jour}`, congés manuels `timeOff.{jour}`), rémunération séparée `employeesComp` (admin seulement, fusionnée à l'affichage), `settings/schedule` (jours ouverts, ratio, ordre et employés retirés **par semaine**), congés approuvés de `leaveRequests`. **Calculs identiques à la v1** : heures (quart qui passe minuit compris), taux horaire **daté** (historique de taux, taux du jour pour les employés à l'heure, taux du 1er jour ouvert × heures fixes réparti sur les jours ouverts pour les salariés), ventes prévues = coût ÷ ratio. **Tests de parité v1 ↔ v2** (nouveau, `app/src/test/v1.ts`) : les fonctions de la v1 sont extraites de `js/*.js` et comparées à la v2 sur des centaines de cas générés (heures, taux datés, historique, couverture par heure, plages de congé, saisie d'heure) — ignorés automatiquement si le dossier `js/` n'existe plus. Écran : grille semaine (colonne employé et totaux fixes au défilement), clic sur une case = ajouter / modifier / **déplacer** un quart (choix du jour au lieu du glisser-déposer, fonctionne sur tablette), congé manuel sur une plage (les quarts ces jours-là sont retirés, annulable), congé approuvé → renvoi vers « Demandes de congé », « Ajouter des heures quand même » un jour de congé, **Annuler** les 5 dernières actions (Ctrl/Cmd+Z), monter/descendre et retirer un employé pour la semaine, copier vers la semaine suivante (jours de congé respectés, ordre copié), jours d'ouverture, ratio, totaux heures/coût/ventes prévues, **couverture en carte de chaleur** (employés présents par heure et par jour, avec le nombre dans chaque case — remplace les 7 séries de barres de la v1), fiches équipe, archivés + restaurer, fiche employé avec historique des taux, images PNG équipe (sans $) et admin (html2canvas au clic). **Amélioration de fiabilité** : les quarts et congés sont maintenant écrits jour par jour (`shifts.2026-09-25`) au lieu de réécrire tout l'horaire de l'employé — deux admins qui modifient des jours différents ne s'écrasent plus. **Corrigé** : « Dupliquer » un employé copiait aussi son NIP (deux employés avec le même NIP) et tous ses quarts ; la copie n'a plus ni NIP ni quarts. Non repris : l'import « horaire type » codé en dur (`BOCHICA_SCHEDULE_TEMPLATE`, plus relié à l'écran).
>
> ⚠️ **25 septembre 2026 — v2 : étape 4 — espace employé + espagnol** — **Traduction FR/ES** (`app/src/core/i18n/i18n.tsx`) : bouton FR/ES dans le menu et sur l'écran de connexion, choix mémorisé sur l'appareil (même clé que la v1, `bochica-ui-lang`) ; chaque module déclare ses textes `{ fr, es }` et TypeScript refuse une traduction manquante. Traduits : menu, connexion, fenêtres/confirmations communes, et tout ce que voit l'employé (Accueil, Inventaire, Tâches, Ouverture/Fermeture, Mon horaire, Demande de congé). Les pages réservées à l'admin restent en français. **Nouveaux modules** — `modules/operations/` : **Tâches du jour** (admin, collection `dailyTasks` : récurrentes/temps mort, « 1 fois », plusieurs heures par jour = plusieurs passages à cocher ; heures acceptées « 17h », « 1700 », « 17:00 » et remises en ordre ; « Déplacer vers… » au lieu du glisser-déposer), **Tâches** (employé, cochables, remises à zéro à minuit — la date se met à jour toute seule même si la tablette reste ouverte), **Ouverture / Fermeture** (admin + employé, `settings/openClose` + `dailyChecklistState/{date}` ; section Cuisine/Service mémorisée sur la tablette ; éditeur admin 4 zones qui garde les cases cochées quand un texte ne change pas), **Tâches Kanban** (admin, `tasks` : 3 colonnes, bouton « colonne suivante », date limite dépassée en rouge). `modules/equipe/` : **Accueil employé** (qui travaille aujourd'hui par section, prochains événements, tâches du jour cochables — aucun $), **Mon horaire** (même liste et même ordre que l'horaire admin : employés masqués exclus, ordre de la semaine ; congés approuvés et congés partiels affichés ; « Qui es-tu ? » mémorisé sur l'appareil pour voir sa propre semaine en liste), **Demande de congé** (NIP comme le pointage, règle des 2 semaines identique, congé partiel, « Mes demandes » ; retour automatique au clavier après 2 min sans activité pour la tablette partagée), **Demandes de congé** (admin : filtres, approuver/refuser/retirer, avertissement si l'employé a déjà un quart ce jour-là). Même format de données que la v1 partout (`leaveRequests`, `dailyTasks`, `tasks`…), règles Firestore inchangées (les employés n'écrivent que les champs de complétion autorisés). Tests : 106 au total (règles des tâches du jour, NIP, 2 semaines, semaines ISO, heures). **Il ne reste en v1 que** : Employés & Horaires, Salaires & Pourboires, Simulation paie, Pointage (étape 5), et le Tableau de bord admin.
>
> ⚠️ **25 septembre 2026 — v2 : étape 3 — « Dépenses & Revenus », « Factures » et « TPS/TVQ » migrés** — `app/src/modules/finances/` (3 modules, même domaine). Mêmes collections et mêmes champs que la v1 (`expenses`, `revenues`, `expenseCategories`, `fixedExpenseTemplates`, `invoices`, `taxRemittances`, `suppliers`). **Dépenses & Revenus** : périodes 7 jours / mois / année avec navigation, KPI (revenus, dépenses + taxes, profit calculé comme la v1 = revenus − dépenses taxes incluses, marge, fixes/variables), graphique 6 mois en **colonnes groupées sur un seul axe** (revenus vert, dépenses orange) et dépenses par catégorie en barres classées, chacun avec vue Tableau ; listes revenus/dépenses (recherche sans accents, filtre catégorie, ⋮ Modifier/Dupliquer/Supprimer) ; fenêtre dépense identique (fournisseur avec suggestions + **création automatique du fournisseur** comme la v1, nouvelle catégorie en ligne, frais supplémentaires, « aucune taxe », TPS/TVQ auto ou manuelles) ; catégories, frais fixes mensuels, **rapport Excel/PDF** (mêmes onglets et colonnes que la v1 ; xlsx et jspdf-autotable sont des paquets npm chargés au clic). **Frais fixes : la v1 continue de les créer automatiquement ; la v2 affiche seulement une bannière « Créer maintenant »** (pour éviter les doublons tant que les deux apps tournent) — à rendre automatique à la bascule (étape 6). **Factures** : KPI (facturé, payé, en attente dont en retard, brouillons), filtres par statut avec compteurs, cartes, changement de statut direct, fenêtre avec lignes et **totaux en direct**, PDF identique à la v1 (lettre, en-tête tricolore, blocs client/détails, pied de page) avec montants « 1 234,56 $ ». Lien avec les revenus : payé → revenu créé (`paidRevenueId`), dé-payé → revenu retiré, **facture payée modifiée → revenu lié mis à jour** (la v1 ne le faisait pas), TPS/TVQ du revenu arrondies au cent, suppression d'une facture payée → revenu retiré, dupliquer → nouveau numéro FAC-AAAA-NNN, brouillon, échéance +30 j. **TPS/TVQ** : 4 derniers trimestres (✓ si remis), perçues/payées/à remettre, date limite et jours restants, « Marquer comme remis » (même document `taxRemittances` que la v1). **Correctif `firestore.rules`** : ajout de `match /taxRemittances/{doc=**} { allow read, write: if isAdmin(); }` — sans cette règle, le bouton « Marquer comme remis » de la v1 échouait en silence (refus par défaut) ; **à publier dans la console Firebase**. Autres corrections v1 : les dépenses datées du 1er du mois n'étaient pas toujours comptées dans le bon mois du graphique 6 mois (date lue en UTC) ; trimestres calculés en date locale. Graphiques partagés déplacés dans `app/src/ui/charts/` (ChartCard, ColumnsChart, RankedBarsChart). Le **Tableau de bord** (prévu à l'étape 3) est reporté : il dépend des employés, salaires et tâches (étapes 4-5).
>
> ⚠️ **25 septembre 2026 — v2 : étape 2 — « Événements » et « Soumissions » migrés** — **Événements** (`modules/evenements/`) : même collection `events`, mêmes types (réservation, karaoké, spectacle, hors Bochica, férié, interne ; ancien type « special » affiché) et statuts. Calendrier mensuel (lundi → dimanche) avec **une couleur fixe par type** (palette validée) + icône, « en attente » en pointillé, « annulé » barré ; clic sur un jour = liste des événements du jour sous le calendrier + « Ajouter ce jour-là » (double-clic = ajout direct) ; sur téléphone, pastilles de couleur au lieu du texte. Vues « Liste du mois » et « À venir (30 j) ». **Avertissement quand un autre événement existe déjà le même jour.** Téléphone et courriel cliquables. Dates en texte local AAAA-MM-JJ comme la v1 (`core/dates.ts`, testé). **Soumissions** (`modules/soumissions/`) : mêmes collections `quotes` + `quoteTemplates`, même format (options `packageOptions[]` avec forfait figé, `roomRentals[]`, **champs hérités** de la 1re option toujours écrits, lecture des anciennes soumissions à un seul forfait). **Calculs identiques à la v1** (forfait × personnes + bière + café/dessert + lignes libres, TPS 5 % + TVQ 9,975 % sans arrondi intermédiaire, dépôt/solde) — couverts par des tests (`core/taxes.ts` partagé pour les Finances). Fenêtre de soumission avec **totaux en direct pour chaque option**. Numérotation AAAA-NNN identique. « Expirée » affichée automatiquement quand la validité est dépassée. **PDF** : même mise en page que la v1 (en-tête, tricolore, cartes forfait, options à cocher, location de salle, notes, QR code du menu, pages numérotées) ; jsPDF et le QR sont maintenant des paquets npm chargés seulement au clic ; montants au format québécois « 1 234,56 $ » (la v1 affichait « 1234.56 $ »). Corrections v1 : **dupliquer une soumission lui donne maintenant un nouveau numéro** (la v1 gardait le même) et la remet en brouillon ; la suppression d'un forfait passe par la fenêtre de confirmation de l'app. Forfaits par défaut (L'Essentiel, Le Gourmand) créés d'un clic si la collection est vide (plus de création automatique au chargement).
>
> ⚠️ **25 septembre 2026 — v2 : fin de l'étape 1 — « Liste d'ingrédients », « Menu », « Ingrédients », « Recettes » migrés** — Mêmes collections et mêmes champs que la v1 (`shoppingList`, `menu` avec `recipe[{ingredientId, qty}]`, `ingredients`, `recipes` en markdown). **Liste d'ingrédients** (`modules/liste-ingredients/`) : fournisseurs fixes Costco/Viandex/Gordon conservés (codes v1 ; à rendre configurables avant la vente), filtre par fournisseur avec compteurs, recherche sans accents, tri par fournisseur ou A→Z, **détection des doublons** chez le même fournisseur. **Cuisine** (`modules/cuisine/`, 3 modules) : **Menu** — coût de revient identique à la v1 (coût unitaire × quantité, ingrédient supprimé = 0), seuils de marge v1 (≥ 70 % bonne, ≥ 50 % correcte) avec étiquette icône + texte, **coût matière en %** et **prix suggéré pour 30 % de coût matière** (bouton « Utiliser »), case « Disponible » dans la fenêtre. Correction v1 : modifier un plat indisponible ne le rend plus disponible par erreur. **Ingrédients** — coûts unitaires jusqu'à 4 décimales (0,0180 $/g au lieu de 0,02 $), nombre de plats qui utilisent chaque ingrédient, avertissement avant suppression et quand un changement de coût touche des plats. **Recettes** — lecture en grand format, impression, éditeur avec barre gras/italique/barré/listes + Ctrl/Cmd+B/I ; le markdown est maintenant rendu **sans HTML brut** (`ui/Markdown.tsx` + `ui/mdParse.ts`, même syntaxe que la v1, rétrocompatible avec les textes sans marques). ⚠️ Le Mac a un système de fichiers insensible à la casse : ne jamais créer deux fichiers qui ne diffèrent que par la casse (ex. `markdown.ts` et `Markdown.tsx`) — Vercel (Linux) les distingue. Bancs d'essai visuels : `app/.harness/` (ignoré par git).
>
> ⚠️ **25 septembre 2026 — v2 : « Inventaire » et « À commander » migrés** — `app/src/modules/inventaire/` (deux modules dans le même dossier, même domaine). Mêmes collections et mêmes champs que la v1 (`products`, `suppliers`, `settings/sections` avec `all` + `custom` préservé). **Règles de statut identiques** (rouge ≤ minimum, jaune ≤ minimum × 1,2, sinon vert ; tri statut puis `sortOrder`) — couvertes par des tests. Inventaire : tuiles de comptage, onglets de catégories avec pastille des produits à surveiller, recherche sans accents (nom + note), saisie du nouveau stock (Entrée ou ✓, journal « Mise à jour stock » identique), menu ⋮ (Modifier, Dupliquer — stock remis à 0 comme en v1 —, Archiver/Restaurer, Supprimer), vue des archivés, fenêtre produit unique (la note et la catégorie y sont maintenant, au lieu de fenêtres séparées), fenêtre « Gérer les catégories » (ajouter, renommer → met à jour les produits, supprimer → déplace vers « Autre », réordonner avec ↑↓ au lieu du glisser-déposer). Droits : admin et chef gèrent, employé voit et met à jour le stock seulement. À commander (admin) : regroupement par catégorie ou par fournisseur (choix mémorisé), fournisseurs supprimés et « Sans fournisseur » à la fin, téléphone cliquable, **Réceptionner** avec aperçu (unités, nouveau stock, écart vs prévu ; journal « Réception » identique), **Imprimer** via l'impression du navigateur (menu masqué à l'impression). **Pas encore porté** : réordonner les produits par glisser-déposer (toujours possible dans la v1) ; l'espagnol (les employés doivent rester sur la v1 jusqu'à l'étape 4).
>
> ⚠️ **25 septembre 2026 — v2 : étape 0 (fondations) + module « Fournisseurs » migré** — Briques communes de la v2 : écritures Firestore via le tenant (`core/data/mutations.ts`, `useDataActions` : create/update/remove + **journal `/logs` au même format que la v1**, avec `source: "v2"`), fenêtre modale accessible (`<dialog>` natif), confirmation `await confirm(...)`, notifications, champs de formulaire avec erreurs, recherche. **Tests automatiques Vitest** (`npm test`) — ils tournent aussi à chaque build Vercel : si un calcul casse, le déploiement est bloqué. **Fournisseurs** (`app/src/modules/fournisseurs/`) : même collection `suppliers`, mêmes champs (name, contact, email, notes) + `createdAt/updatedAt`. Nouveautés : recherche (sans accents, y compris dans les produits liés), tri alphabétique, téléphone et courriel cliquables, validation (nom obligatoire et unique, courriel valide), duplication qui ouvre directement la copie, confirmation de suppression qui indique combien de produits sont liés. Traduction espagnole : reportée à l'étape 4 (espace employé), les modules admin sont en français.
>
> ⚠️ **25 septembre 2026 — v2 : module « Rapports mensuels » migré en React** — Premier vrai module de la v2 (`app/src/modules/rapports/`), en lecture seule sur la même collection `monthlyReports`. Graphiques refaits avec **Recharts** (chargé seulement sur cette page) selon des règles de lisibilité : **plus de graphique à double axe** (Ventes et Pourboires étaient superposés avec 2 échelles → maintenant 3 graphiques séparés : Ventes, Pourboires, Heures), **plus de beigne à 12 parts** (catégories et modes de paiement → barres horizontales classées avec montant + %, la queue regroupée en « Autres »), canaux de vente en barres empilées avec **une couleur fixe par canal** (Ramassage salle + en ligne regroupés). Palette « Bochica » (jaune, bleu, orange, aqua, violet, magenta) **validée pour le daltonisme** en clair et en sombre (`app/src/ui/chartTheme.ts`). Chaque graphique a un bouton **Graphique / Tableau**. Correction de logique : l'écart vs l'année précédente des KPI compare maintenant **les mêmes mois** (avant : total de la période complète vs total A-1 partiel) et l'affiche (« sur 4 mois comparables : X vs Y »). Mois sans heures = « — » au lieu de « 0 h ». Calculs purs dans `rapports.logic.ts`. L'import seed reste dans la v1.
>
> ⚠️ **25 septembre 2026 — Démarrage de la v2 (React) dans `app/`** — **Début de la migration vers une architecture « monolithe modulaire » en React + TypeScript + Vite**, dans le même dépôt, sur la même base Firestore. Objectif : sortir du code global partagé, pouvoir changer les graphiques, et préparer une vente future à d'autres restaurants (multi-restaurant). **L'ancienne app (racine) n'est pas touchée** et reste en production. Squelette livré : connexion Firebase Auth (mêmes comptes/rôles, accepte le nom d'utilisateur ou le courriel), menu généré depuis un **registre de modules** (`app/src/modules/registry.ts`), badge « V1 » + lien vers l'ancienne app pour les modules pas encore migrés, thème clair/sombre avec les tokens Bochica, couche de données **tenant-aware** (`app/src/core/data/` — aucun nom de collection en dur dans les pages ; tenant Bochica en `layout: "root"`, cible `restaurants/{id}/…`). Seul module migré : **Accueil**. Prochain module prévu : **Rapports mensuels** (lecture seule, nouveaux graphiques). Déploiement : 2ᵉ projet Vercel avec *Root Directory* = `app`. Voir `app/README.md` et `app/src/modules/README.md`.
>
> ⚠️ **Dernière mise à jour : 20 septembre 2026 — v3.81.0** — **Le correctif v3.80.0 n’avait aucun effet sur Salaires & Pourboires : une règle CSS plus spécifique propre à cette page écrasait la règle générique**. En vérifiant le correctif précédent en direct dans le navigateur (mesures `getBoundingClientRect`/`getComputedStyle`), la colonne « Totaux » faisait toujours 140px au lieu des 180px attendus : `.schedule-totals-panel.payroll-totals-panel .schedule-totals-grid` (ajoutée en v3.27.0 pour adapter les largeurs de colonnes à cette page précise) a une spécificité CSS plus élevée que la règle générique modifiée en v3.80.0, et fixait encore sa dernière colonne à 140px (110px en mobile) — ce qui explique pourquoi le « carré » restait cassé malgré le premier correctif. **Correctif (`style.css`)** : `.schedule-empgrid.payroll-empgrid` et `.schedule-totals-panel.payroll-totals-panel .schedule-totals-grid` (desktop) passent à 190px pour la dernière colonne (au lieu de 140px), et leur équivalent mobile (≤900px) à 150px (au lieu de 110px) — la colonne « Total » par employé au-dessus garde la même largeur que le panneau de totaux pour rester alignée (ses libellés sont courts, elle a juste plus d’air). Purement visuel — aucun calcul touché. Banc d’essai : CSS relu (accolades équilibrées, 2289/2289) ; vérifié en direct dans le navigateur connecté — le bloc `.payroll-totals-final` passe de 169px à 86.5px de hauteur et ses 3 lignes (Sans pourb. / Pourboires / Avec pourb.) tiennent chacune sur une seule ligne (15px de hauteur de libellé, contre 30px = 2 lignes avant le correctif). `CACHE_VERSION` → `v3.81.0`.
>
> ⚠️ **20 septembre 2026 — v3.80.0** — **Panneau « Totaux semaine » (Salaires & Pourboires) : dernière colonne de la grille élargie de 96px à 180px (règle générique)**. Demande : dans le panneau de totaux sous la grille employés×jours, le bloc de droite (« Sans pourb. / Pourboires / Avec pourb. ») était coincé dans une colonne de seulement 96px, ce qui faisait retomber les libellés et les montants sur 2 lignes chacun — l’utilisateur a demandé plus d’espace horizontal pour que ce « carré » prenne moins de place verticalement. **`style.css`** : la règle générique partagée par `.schedule-empgrid`, `.schedule-totals-grid` et `.schedule-totals-day-row` passe de 96px à 180px pour la dernière colonne (desktop), et de 78px à 140px en mobile (≤900px) — ces 3 sélecteurs doivent rester identiques pour que les colonnes s’alignent verticalement entre la grille et son panneau de totaux. Banc d’essai : CSS relu (accolades équilibrées, 2289/2289). `CACHE_VERSION` → `v3.80.0`.
>
> ⚠️ **20 septembre 2026 — v3.79.0** — **Légende Cuisine/Service/Autre repliée dans l'en-tête "Employé" des grilles empgrid, au lieu de flotter au-dessus de tout le tableau**. Demande : sur Salaires & Pourboires, la légende de couleurs (points Cuisine/Service/Autre) était affichée juste au-dessus de la grille employés×jours, sans lien visuel avec la colonne "Employé · Section" qu'elle explique en réalité — l'utilisateur a demandé de la descendre directement dans l'en-tête de cette colonne. `renderSectionLegend()` (fonction partagée depuis la v3.63.0, réutilisée sur Horaires, Salaires & Pourboires et Simulation paie) est déplacée du dessus de `.schedule-empgrid` vers l'intérieur de `.schedule-empgrid-emp-head`, sous le libellé de colonne ("Employé", "Employé · Section" ou "Employé · Taux · Section" selon la page) — même changement appliqué aux 3 pages pour rester cohérent, puisque c'est le même composant partagé. **`style.css`** : `.schedule-empgrid-emp-head` passe en colonne flex (label + légende empilés) ; nouvelle règle `.schedule-empgrid-emp-head .section-legend` qui compacte la légende à cet endroit (retire le padding du dessus, réduit la taille de police à 9px et les points à 6px, annule l'uppercase hérité de l'en-tête de colonne) — la légende garde son `flex-wrap` et retombe sur 2 lignes sous le libellé si l'espace de 160px ne suffit pas sur une seule ligne, sans casser la grille. Purement visuel — aucun calcul touché. Banc d'essai : `node --check` OK (`pages-hr.js`, `pages-payroll.js`, `pages-simulations.js`) ; CSS relu (accolades équilibrées, 2289/2289). `CACHE_VERSION` → `v3.79.0`.
>
> ⚠️ **20 septembre 2026 — v3.78.0** — **Cartes "collées" (round 5, cause réelle trouvée) : retour à `display:grid`+`gap`, la technique "rows" en marges négatives causait un chevauchement**. Après 4 correctifs successifs (v3.73.0 à v3.77.0 : gap, bordures, ombres, puis inversion du contraste carte/tuiles), le problème persistait — l'utilisateur a connecté Chrome et inspecté la page en direct avec les DevTools, révélant la vraie cause : `.payroll-tips-grid` avait `margin: -16px 0px 0px -16px`, qui faisait chevaucher la grille des 7 jours par-dessus la rangée du dessus. Cette technique "rows" (marge négative sur le conteneur flex + marge positive sur chaque tuile), adoptée en v3.75.0/v3.76.0 sur suggestion de l'utilisateur, n'est sécuritaire que pour un conteneur qui est le tout premier élément sans frère précédent dans le flux normal — appliquée à `.stat-grid` et `.payroll-tips-grid`, qui suivent chacun un élément frère, la marge négative en haut entrait en collision avec l'élément précédent et tirait toute la grille vers le haut, la faisant chevaucher visuellement. C'était donc le vrai bug derrière "les cartes collées" depuis le début — jamais un problème de couleur, de contraste ou de taille d'espacement comme supposé précédemment (l'inversion de contraste en v3.77.0 restait un bon choix en soi, mais ne réglait pas le chevauchement). **Correctif (`style.css`)** : `.payroll-overview-card .stat-grid`/`.stat-card` et `.payroll-tips-grid`/`.payroll-tips-day` reviennent à `display:grid`+`gap` (20px et 16px respectivement) sans aucune marge négative — la technique qui avait été abandonnée par précaution en v3.75.0/v3.76.0, sans que la prémisse ("gap" peu fiable) n'ait jamais été la vraie cause. Purement visuel — aucun calcul touché. **Méthode de diagnostic** : après plusieurs rounds de correctifs devinés à partir de captures d'écran sans succès, connexion de Claude in Chrome au navigateur réel de l'utilisateur (via un serveur local lancé manuellement, `python3 -m http.server`) pour inspecter la page en direct — c'est l'utilisateur qui a repéré la marge fautive dans l'inspecteur DevTools. Banc d'essai : CSS relu (accolades équilibrées, 2287/2287) ; vérifié en direct dans le navigateur connecté via `getBoundingClientRect`/`getComputedStyle` — écart réel de 16px entre les deux rangées (était -16px, chevauchement, avant le correctif), 20px entre chaque tuile, `display:grid` confirmé sur les deux grilles. `CACHE_VERSION` → `v3.78.0`.
>
> ⚠️ **20 septembre 2026 — v3.77.0** — **Cartes "collées" (round 4) : inversion du contraste carte/tuiles au lieu d'ajuster encore l'espacement**. Après 3 correctifs successifs basés sur `gap`/marges/bordure/ombre (v3.73.0 à v3.76.0) sans que l'utilisateur ne perçoive de vraie séparation, question posée directement : le vrai problème identifié ensemble est que la carte `.payroll-overview-card` (blanche, `var(--surface)`) ET ses tuiles (beiges, `var(--surface2)`) restaient deux tons de beige très proches du fond de page (`var(--bg)`, aussi crème) — un espace de 16-20px entre les tuiles ne suffisait pas à créer un contraste perceptible à l'œil, peu importe sa taille. **Correctif choisi (sur 4 options proposées) : fond blanc pur pour les cartes.** `style.css` : `.payroll-overview-card` passe en fond beige (`var(--surface2)`, devient le "panneau") tandis que ses tuiles (`.stat-card`) ET les 7 cases du jour (`.payroll-tips-day`) passent en blanc franc (`var(--surface)`) — inversion du sens du contraste plutôt qu'un simple ajustement de degré. Les tuiles/cases "flottent" maintenant clairement sur un fond plus sombre, ce qui fonctionne indépendamment de la taille exacte de l'espace entre elles. Les variantes teintées (`.payroll-tips-day--is-good/--is-bad`) restent inchangées et ressortent même mieux sur fond blanc. En dark mode le sens s'inverse naturellement dans la bonne direction aussi (`--surface` plus clair que `--surface2` en dark, donc tuiles toujours plus claires que le panneau). Purement visuel — aucun calcul touché. Banc d'essai : CSS relu (accolades équilibrées, 2287/2287). `CACHE_VERSION` → `v3.77.0`.
>
> ⚠️ **20 septembre 2026 — v3.76.0** — **Ratio salaires/ventes : source unifiée sur les ventes nettes de Salaires & Pourboires (fin de la double saisie « Ventes réelles » dans Employés & Horaires) + grille des 7 jours repassée en "rows"**. Demande explicite : les "ventes réelles" étaient saisies à 2 endroits différents (`scheduleSettings.actualSales` dans Employés & Horaires, ET `netByDay` dans Salaires & Pourboires) sans lien entre les deux, ce qui expliquait pourquoi la tuile "Ratio salaires/ventes" restait vide malgré des ventes nettes saisies sur l'autre page. **(1) `pages-payroll.js`** : les 3 endroits qui lisaient `scheduleSettings.actualSales` (`renderSalaires()`, `generatePayrollPDF()`, `_computePayrollWeekData()`) utilisent maintenant `netByDay`/`totalNet` — la même source déjà saisie pour la rentabilité et le % pourboire sur cette page. `_computePayrollWeekData()` expose maintenant aussi `netByDay`/`totalNet` dans son objet retourné, par cohérence avec `tipsByDay`/`totalTips`. **(2) `pages-hr.js`** : le champ de saisie "Ventes réelles" + la ligne "Écart" (qui en dépendait) sont retirés du panneau de totaux de la page Employés & Horaires — "Ventes prévues" (calculée, pas saisie) reste inchangée. La tuile "Ventes réelles" de l'export PNG de l'horaire est retirée aussi (4 tuiles → 3 : Heures, Masse salariale, Ventes prévues). La fonction `updateActualSales()`, devenue orpheline, est supprimée. **(3) `style.css`** : la grille des 7 jours (`.payroll-tips-grid`/`.payroll-tips-day`) reçoit le même traitement "rows" flex + marges que le stat-grid en v3.75.0 (bordure/ombre renforcées) — elle utilisait encore `display:grid`+`gap:8px` avec une bordure à 10% d'opacité, nettement plus discrète que le stat-grid, d'où l'impression de cartes encore "collées" en comparaison. Classes CSS `.schedule-sales-input`/`.schedule-totals-val--gap`/`.gap-arrow` devenues orphelines sur la page Employés & Horaires mais laissées en CSS (dette mineure, aucun risque). **⚠️ Non traité, à valider avec l'utilisateur** : le widget "Ratio salaires/ventes" du Dashboard (`pages-dashboard.js` ~ligne 500, `dash-today-widget__ratio-pct`) lit aussi `scheduleSettings.actualSales` et affichera désormais toujours vide/— , puisque plus rien n'alimente ce champ. Le corriger proprement demanderait d'abonner le Dashboard aux données de Salaires & Pourboires (changement d'architecture non demandé) — signalé à l'utilisateur, décision en attente. Banc d'essai : `node --check` OK (`pages-payroll.js`, `pages-hr.js`) ; CSS relu (accolades équilibrées, 2286/2286) ; vérifié qu'aucune autre page ne dépend de `actualSales`/`updateActualSales` en dehors du Dashboard signalé ci-dessus. `CACHE_VERSION` → `v3.76.0`.
>
> ⚠️ **20 septembre 2026 — v3.75.0** — **Salaires & Pourboires : rangée Pool Cuisine/Service retirée (pliée dans la tuile Pourboires) + tuiles du haut repassées en "rows" flex plutôt qu'en grid-gap**. Suite à une nouvelle capture montrant que (a) la rangée pleine largeur "Total pourboires / Pool Cuisine / Pool Service" prenait trop de place pour l'info qu'elle donnait, et (b) les tuiles du haut restaient perçues comme "collées" malgré 2 correctifs successifs basés sur `gap`. **(1) Rangée pools retirée** (`pages-payroll.js`/`style.css`) : le bloc `.payroll-tips-pools` (3 colonnes en bas de la carte fusionnée) est supprimé ; son contenu (Pool Cuisine, Pool Service — montant + % + heures éligibles en info-bulle) est plié directement dans la tuile "Pourboires" du `.stat-grid`, sous un nouveau sous-bloc `.payroll-overview-pools` (2 lignes compactes, séparateur pointillé, couleurs de section réutilisées : cuisine `#BA7517`, service `#378ADD`). Le CSS `.payroll-tips-pools`/`.payroll-tips-pool*` (+ overrides dark) devenu orphelin est retiré (aucune autre page n'y référait). **(2) Tuiles en "rows"** : sur suggestion de l'utilisateur ("tu devrais peut-être le faire avec des rows"), `.payroll-overview-card .stat-grid`/`.stat-card` abandonnent `display:grid`+`gap` au profit de la technique flexbox classique par marges (marge négative sur le conteneur, marge positive sur chaque tuile) — la séparation entre tuiles ne dépend plus du tout de `gap`, ce qui élimine toute incertitude de rendu liée à ce paramètre. Purement de l'affichage — aucun calcul ni donnée touchée. Banc d'essai : `node --check` OK (`pages-payroll.js`) ; CSS relu (accolades équilibrées, 2285/2285) ; aucune référence orpheline aux classes retirées. `CACHE_VERSION` → `v3.75.0`.
>
> ⚠️ **20 septembre 2026 — v3.74.0** — **Salaires & Pourboires : les tuiles du haut restaient "collées" malgré le correctif v3.73.0 + fusion Pourboires/% Pourboire**. Après capture d'écran montrant que le contraste de fond seul (v3.73.0) ne suffisait pas à détacher visuellement les tuiles (bordure à 10% d'opacité et ombre quasi invisible héritées de `.stat-card`), 2 correctifs sur `pages-payroll.js`/`style.css`. **(1) Séparation renforcée** : `gap` du `.stat-grid` élargi une 2e fois (`var(--sp-4)` → `var(--sp-5)`) ET bordure/ombre des tuiles nettement plus marquées (`border-color:var(--border-strong)` au lieu de 10% d'opacité, vraie `box-shadow` au lieu de `--shadow-sm` quasi invisible, variante dark mode ajoutée) — les 3 leviers combinés pour que chaque tuile se détache clairement peu importe la teinte de fond. **(2) Fusion Pourboires + % Pourboire** : ces 2 tuiles redondantes (un montant, un pourcentage du même chiffre) deviennent UNE seule tuile — la tuile "Pourboires" affiche maintenant `$X · Y% des ventes nettes` en sous-texte, même format que la tuile "Total pourboires" déjà présente dans la carte Pool Cuisine/Service plus bas. Le `.stat-grid` passe donc de 6 à 5 tuiles (Ratio, Salaires payés, Pourboires, Ventes nettes, Rentabilité — cette dernière toujours en dernière position, décidé en v3.73.0). Purement de l'affichage — aucun calcul ni donnée touchée. Banc d'essai : `node --check` OK (`pages-payroll.js`) ; CSS relu (accolades équilibrées, 2291/2291). `CACHE_VERSION` → `v3.74.0`.
>
> ⚠️ **20 septembre 2026 — v3.73.0** — **Salaires & Pourboires : retouches visuelles sur la carte fusionnée (contraste, ordre, couleur par jour, fusion des pools)**. Suite aux captures d'écran envoyées après la fusion v3.72.0, 4 ajustements demandés sur `pages-payroll.js`/`style.css`. **(1) Tuiles "collées"** : les 6 `.stat-card` du haut avaient le même fond que la carte `.payroll-overview-card` (`var(--surface)` des deux côtés), ce qui les faisait paraître soudées malgré le `gap`. Fond passé à `var(--surface2)` (comme les cases du jour) + `gap` élargi (`var(--sp-3)` → `var(--sp-4)`), scopé à `.payroll-overview-card .stat-card`/`.stat-grid` pour ne pas toucher les autres `.stat-grid` de l'app (Dashboard, Dépenses…). **(2) Rentabilité toujours à droite** : la tuile "Rentabilité — % atteint" est déplacée en dernière position (après % Pourboire) dans le `.stat-grid`, pour rester visible à droite plutôt qu'au milieu. **(3) Couleur par jour** : chaque case de `.payroll-tips-grid` reçoit maintenant la classe `payroll-tips-day--is-good`/`--is-bad` (même logique `dCls` déjà calculée pour le sous-bloc rentabilité) avec un fond + bordure teintés vert/rouge sur TOUTE la case, pas seulement le sous-bloc du bas — permet de repérer les bonnes/mauvaises journées d'un coup d'œil. **(4) Fusion Pool Cuisine + Pool Service** : les 2 cartes séparées deviennent une seule carte `.payroll-tips-pools` à 3 colonnes (liserés internes au lieu de 2 boîtes distinctes) : Total pourboires (nouvelle tuile, $ + % des ventes nettes) + Pool Cuisine + Pool Service — reprend les couleurs de section déjà utilisées ailleurs dans l'app (cuisine `#BA7517`, service `#378ADD`) en bordure du haut de chaque colonne. Purement de l'affichage — aucun calcul ni donnée touchée. Banc d'essai : `node --check` OK (`pages-payroll.js`) ; CSS relu (accolades équilibrées, 2290/2290). `CACHE_VERSION` → `v3.73.0`.
>
> ⚠️ **20 septembre 2026 — v3.72.0** — **Salaires & Pourboires : fusion de la carte "Vue d'ensemble financière" et de la carte "Pourboires & ventes nettes" en une seule carte**. Suite à la question « on peut merger ces 2 sections ? » (avec la demande d'y intégrer aussi la saisie du pourboire/net dans la vue d'ensemble), l'utilisateur a approuvé la fusion complète (`pages-payroll.js`). **(1) 3 tuiles ajoutées au `.stat-grid`** de `.payroll-overview-card`, qui passe de 3 à 6 `.stat-card` : Pourboires (total semaine, bordure accent doré), Ventes nettes (total semaine, bordure bleu `#3b7cc6`), % Pourboire (bordure verte) — réutilisent les variables déjà calculées plus haut dans `renderSalaires()` (`totalTips`, `totalNet`). L'info-bulle « Détail des calculs » de l'en-tête a été étendue avec la formule du % Pourboire. **(2) Grille par jour unifiée** : les deux grilles séparées (`.payroll-tips-grid` — pourboire + net uniquement — et `.payroll-profit-days` — rentabilité uniquement) deviennent UNE seule `.payroll-tips-grid` de 7 cases ; chaque case combine maintenant nom du jour, input pourboire, input ventes nettes, % pourboire du jour, ET la rentabilité du jour (% atteint + surplus/manquant, vert/rouge) dans un nouveau sous-bloc `.payroll-tips-day__profit` séparé par un trait pointillé — reprend `dayProfitList[k]` (même index que `weekDays`, vérifié). **(3) Pools Cuisine/Service** (`.payroll-tips-pools`) conservés tels quels en bas de la carte fusionnée, inchangés. L'ancien en-tête + rangée de totaux de `.payroll-tips-card` (devenus redondants avec les nouvelles tuiles) et le bloc `.payroll-tips-card` lui-même sont retirés ; la carte d'alertes (`.payroll-alerts-card`, conditionnelle) qui s'intercalait entre les deux anciennes cartes se retrouve maintenant juste après la carte fusionnée — même ordre visuel qu'avant. CSS (`style.css`) : `.payroll-tips-grid` élargie (`minmax(140px→160px, 1fr)`) pour accueillir le contenu supplémentaire par case ; nouveau bloc `.payroll-tips-day__profit` (+ `-pct`/`-label`/`-amt`, modificateurs `--is-good`/`--is-bad`/`--is-empty`) qui reprend le langage visuel de `.payroll-profit-day`. Classes `.payroll-tips-head`/`.payroll-tips-total(__label/__amount)`/`--net`/`--pct` et `.payroll-profit-days`/`.payroll-profit-day*` devenues inutilisées sur cette page mais laissées en CSS (aucune autre page n'en dépend, vérifié) — dette mineure sans risque. Purement de l'affichage — aucun calcul ni donnée touchée. Banc d'essai : `node --check` OK (`pages-payroll.js`) ; CSS relu (accolades équilibrées, 2284/2284). `CACHE_VERSION` → `v3.72.0`.
>
> ⚠️ **20 septembre 2026 — v3.71.0** — **Salaires & Pourboires : allègement (suite) — fusion des 3 cartes du haut + légende en info-bulle**. Suite à la demande « qu'est-ce qui peut être allégé sur cette page ? », l'utilisateur a choisi 3 des 4 pistes proposées. **(1) Fusion des 3 cartes du haut** (`pages-payroll.js`) : « Ratio salaires/ventes réelles », « Salaires — estimé vs payé » et « Rentabilité de la semaine » — chacune avec son propre en-tête + long sous-titre toujours affiché — deviennent UNE carte `.payroll-overview-card` avec un seul en-tête et 3 tuiles `.stat-card` (réutilise le composant générique déjà utilisé sur le Dashboard/Dépenses) : Ratio salaires/ventes, Salaires payés (avec Estimé + Écart en sous-texte), Rentabilité — % atteint (avec surplus/manquant en sous-texte). Couleur de bordure par tuile selon le même code good/warn/bad qu'avant (`var(--status-green)`/`#b45309`/`var(--status-red)`). Le détail par jour (`payroll-profit-days`, pastilles % atteint) est **conservé tel quel** sous les tuiles — demandé explicitement par l'utilisateur, pas de doublon retiré cette fois. **(2) Sous-titres → info-bulle** : les phrases de formule (« Salaires bruts ÷ Ventes réelles... ») ne sont plus affichées en permanence — un seul info-tip « Détail des calculs » sur l'en-tête de la carte fusionnée explique les 3 formules au survol. **(3) Légende de bas de grille → info-bulle** : le paragraphe `.payroll-legend` (jaune/bleu, barre ambrée, ★) est remplacé par un tag « Légende de la grille » aligné à droite sous la grille, même composant `.info-tip` que la bannière du haut (v3.70.0) et l'en-tête de la carte fusionnée. CSS ajouté : `.payroll-overview-head`, `.payroll-overview-sub` (`style.css`). Classes `.payroll-ratio-card`/`.payroll-estimate-card`/`.payroll-profit-card`/`.payroll-legend` devenues inutilisées sur cette page mais laissées en CSS (aucune autre page n'en dépend, vérifié) — dette mineure sans risque. Purement de l'affichage — aucun calcul ni donnée touchée. Banc d'essai : `node --check` OK (`pages-payroll.js`) ; CSS relu (accolades équilibrées, 2273/2273). `CACHE_VERSION` → `v3.71.0`.
>
> ⚠️ **20 septembre 2026 — v3.70.0** — **Salaires & Pourboires : bannière d'info remplacée par une info-bulle au survol (allègement de la page)**. Demande utilisateur : alléger la page, notamment le gros bloc de texte explicatif en haut (« Les cellules se remplissent avec les pointages... ») qui prenait de la place en permanence — le remplacer par un petit texte qui affiche l'explication seulement au survol. **Correctif** (`pages-payroll.js`) : la bannière pleine largeur `.payroll-info-banner` est retirée de `renderSalaires()` (toujours utilisée telle quelle sur la page Rapports, `pages-rapports.js`, non touchée) et remplacée par un tag compact « ℹ Comment ça marche ? » juste à côté du titre de la page, dans le `<h2>` — même emplacement que le badge « Payée » existant. **Nouveau composant CSS réutilisable** `.info-tip` / `.info-tip__popup` (`style.css`) : petit tag bleu discret, l'explication complète apparaît dans une bulle positionnée sous le tag au survol (`:hover`) ou au focus clavier (`:focus-within`/`:focus`, tag rendu focusable via `tabindex="0"` — accessible au clavier et au tactile), transition douce, thème sombre géré. Purement une transformation d'affichage — le texte de l'explication n'a pas changé, aucune donnée touchée. Banc d'essai : `node --check` OK (`pages-payroll.js`) ; CSS relu (accolades équilibrées, 2271/2271). `CACHE_VERSION` → `v3.70.0`.
>
> ⚠️ **20 septembre 2026 — v3.69.0** — **Fix graphique « Coût par jour » : couleur Réel/Planifié incohérente avec la légende**. Signalé par l'utilisateur avec capture d'écran : dans le graphique « Coût salarial par jour — réel vs planifié » (v3.68.0), la légende affichait « Réel » en vert, mais certaines barres Réel apparaissaient en rouge (v3.68.0 les colorait dynamiquement rouge/vert selon dépassement du budget CE jour-là). Chart.js n'affiche qu'une seule couleur de légende par série — avec un tableau de couleurs par barre, la légende ne correspondait plus aux barres individuelles, ce qui semblait bugué plutôt qu'informatif. **Correctif** (`pages-payroll.js`, `initPayrollCharts()`) : couleur FIXE par série — Réel toujours vert, Planifié toujours gris — légende et barres toujours cohérentes. Le dépassement budgétaire d'un jour reste lisible par la comparaison visuelle des 2 barres (et par le badge `▲/▼` déjà ajouté sur les cartes de quart en v3.68.0). Banc d'essai : `node --check` OK (`pages-payroll.js`). `CACHE_VERSION` → `v3.69.0`.
>
> ⚠️ **20 septembre 2026 — v3.68.0** — **Salaires & Pourboires : écart quotidien sur les cartes de quart + section « Analyse de la semaine » (2 graphiques Chart.js)**. Deux demandes utilisateur sur la page Salaires & Pourboires. **(1) Écart PAR JOUR** (`pages-payroll.js`, `style.css`) : le total hebdo « Écart » de la cellule Totaux (ex. -2.4h) était déjà la somme des écarts quotidiens (réel − planifié CE jour, pas un seuil fixe de 40h) — mais un jour en manque pouvait être masqué par un autre jour en surplus dans ce seul chiffre net. Ajout d'un petit badge `▲+Xh`/`▼-Xh` (`.shift-card-daygap`) directement sur la carte de quart de CHAQUE jour, quand un quart était planifié ce jour-là et que l'écart est significatif (≥ 0,01h) : sur un quart pointé (réel vs `d.pHours` de ce jour, ignoré si `autoFilled` — sortie pas encore confirmée) et sur un jour complètement manqué (planifié mais rien pointé → `▼-Xh` = la totalité du quart planifié). Le total hebdo existant n'a pas changé. **(2) Section « Analyse de la semaine »** (`pages-payroll.js`, `sidebar.js`, `style.css`) : nouvelle section en bas de page avec 2 graphiques Chart.js (même patron que `initExpenseCharts()`/`initReportsCharts()` — data injectée en JSON via `#payroll-chart-data`, lue par la nouvelle `initPayrollCharts()`, hookée dans `sidebar.js` sur `activePage === "salaires"`) : *Coût salarial par jour — réel vs planifié* (nouveau calcul `dayEstimatedByDk`, miroir de `dayLaborByDk` existant mais sur `pHours` — barre "Réel" verte si ≤ planifié ce jour, rouge si dépassé) et *Vue d'ensemble de la semaine* (barres Ventes nettes vs Masse salariale réelle + ligne pointillée "Ventes nécessaires" pour la cible `scheduleSettings.salesRatio`, réutilise `dayProfitList` déjà calculé pour la carte Rentabilité — même chiffres, vue graphique en plus). CSS : `.payroll-charts-wrap` (grille responsive, réutilise `.card`/`.chart-card`/`.chart-canvas-wrap` déjà en place pour Dépenses/Rapports). **Sécurité** : aucune écriture Firestore ajoutée — uniquement de l'affichage dérivé de données déjà chargées. Banc d'essai : `node --check` OK (`pages-payroll.js`, `sidebar.js`) ; CSS relu (accolades équilibrées, 2261/2261). `CACHE_VERSION` → `v3.68.0`.
>
> ⚠️ **20 septembre 2026 — v3.67.0** — **Fiche employé identique (suite) : cartes « Équipe » et « Pourboires de la semaine par employé »**. Correction de v3.66.0 : le visuel unifié avait été appliqué à la cellule employé de la grille hebdomadaire (empgrid), mais les cartes que l'utilisateur voyait réellement comme « différentes » sont ailleurs — captures d'écran à l'appui, ce sont (1) les cartes **« Équipe »** sous Horaires (`pages-hr.js`, `.team-card`) : sobres, police normale, sans couleur de section ; et (2) les cartes **« Pourboires de la semaine par employé »** sous Salaires & Pourboires (`pages-payroll.js`, `.payroll-recap-card-emp`) : colorées, nom en grosse police d'affiche, bande de couleur cuisine/service. Les infos affichées restent différentes par nature (fiche employé vs récap de paie calculé de la semaine — impossible et non souhaité de tout fusionner), mais le **style visuel** est maintenant le même. **Confirmé avec l'utilisateur** : le style de référence est celui des cartes Pourboires (colorées). **Correctifs** (`pages-hr.js`, `style.css`) : la carte `.team-card` reçoit maintenant une classe de section (`is-kitchen`/`is-service`/`is-other`, calculée comme partout ailleurs depuis `emp.section`) qui lui donne la même bande de couleur + fond teinté que `.payroll-recap-card-emp` (`is-other` est nouveau, gris — le récap pourboires n'a que Cuisine/Service). Le nom (`.team-card__name`) passe de la police de corps en 700 à `var(--font-heading)` (Bebas Neue) à 22px, comme `.payroll-recap-emp-name`. **Portée** : le contenu de chaque carte n'a pas changé (rôle/taux/téléphone/PIN d'un côté, montant/heures de l'autre) — seul le traitement visuel (couleurs, typo du nom) est aligné. Banc d'essai : `node --check` OK (`pages-hr.js`) ; CSS relu (accolades équilibrées, 2253/2253). `CACHE_VERSION` → `v3.67.0`.
>
> ⚠️ **20 septembre 2026 — v3.66.0** — **Fiche employé identique + ordre des employés partagé entre Horaires et Salaires & Pourboires**. Demande utilisateur : (1) la carte employé (nom, taux, indicateur de section) n'avait pas le même visuel entre les 2 pages ; (2) réordonner les employés dans Horaires ne se répercutait pas dans Salaires & Pourboires (deux ordres séparés à maintenir à la main chaque semaine). **Correctif visuel** (`pages-payroll.js`, `style.css`) : la cellule employé de Salaires & Pourboires réutilise maintenant exactement les mêmes styles que Horaires — poignée de glisser-déposer (`.schedule-emp-drag-handle`, au lieu de `.payroll-drag-handle` propre à la page), mise en page du bloc « méta » en colonne (comme `.schedule-empgrid-emp-meta`) au lieu d'une rangée, et poids/couleur du taux horaire identiques. Le sélecteur de dérogation de section (`.payroll-section-select`, fonctionnalité propre à la paie, conservée) prend maintenant un style neutre — texte gris majuscule, comme le badge de section de Horaires — tant qu'aucune dérogation n'est active ; il ne se colore (jaune/bleu/rouge) que lors d'une dérogation réelle (`is-overridden`), pour continuer à la signaler sans complexifier le cas normal. **Correctif ordre partagé** (`pages-payroll.js`) : `getAllPayrollEmployees()` lit maintenant l'ordre dans `scheduleSettings.weekOrder[weekKey]` (`settings/schedule`, la même source que le glisser-déposer de `pages-hr.js`) au lieu du champ `payroll/{weekId}.empOrder` propre à la page ; `payrollRowDrop()` (glisser-déposer dans Salaires), `saveManualEmployee()` (ajout d'un extra) et `doRemoveManualEmployee()` (retrait d'un extra) écrivent/nettoient désormais ce même champ partagé (nouvelle fonction `payrollScheduleWeekKey()` = `dayKey(getWeekStart(payrollWeekOffset))`, identique au format de clé de semaine de Horaires). L'ancien `empOrder` reste lu en repli pour les semaines déjà personnalisées avant ce changement (rétrocompatibilité, aucune migration de données nécessaire) ; le tri par défaut (sans ordre explicite nulle part) est désormais Cuisine → Service → Autre sur les 2 pages, comme Horaires, via `employeeSectionPriority()`. **Portée** : Simulation paie (`pages-simulations.js`) non touchée — hors de la demande initiale. **Sécurité** : aucun changement de règles Firestore ; écriture dans un document déjà utilisé (`settings/schedule`), déjà accessible en écriture depuis les 2 pages. Banc d'essai : `node --check` OK (`pages-hr.js`, `pages-payroll.js`). `CACHE_VERSION` → `v3.66.0`.
>
> ⚠️ **14 septembre 2026 — v3.65.0** — **Rapports de paie (PDF 1 semaine et 2 semaines) : n'affichent plus les employés inactifs**. Demande utilisateur : les rapports PDF de paie affichaient tous les employés retournés par la liste de paie de la semaine, même ceux qui n'ont pas travaillé cette période, ceux archivés (supprimés) ou ceux sans taux/salaire configuré — ce qui alourdissait le rapport de lignes à 0 $ ou sans intérêt. **Correctifs** (`pages-payroll.js`) : (1) **`generatePayrollPDF()`** (rapport 1 semaine) — après le calcul des `empRows`, un filtre retire toute ligne dont `emp.archived` est vrai, dont le taux effectif (`rate`) n'est pas > 0, ou dont `totalHours` (heures réelles pointées cette semaine) n'est pas > 0 ; si la liste filtrée est vide, toast d'avertissement au lieu de générer un PDF vide. (2) **`generateBiWeeklyPDF()`** (rapport 2 semaines) — même filtre appliqué à `aggregatedRows`, mais sur les **heures combinées des 2 semaines** (`hrs1 + hrs2 > 0`) puisqu'un employé peut n'avoir travaillé qu'une des deux ; `sumsCombined` est maintenant recalculé à partir des lignes **filtrées** (et non plus de `w1.sums`/`w2.sums` bruts) pour que les totaux du PDF correspondent exactement aux lignes affichées. **Comportement confirmé avec l'utilisateur** : un employé archivé disparaît toujours du rapport, même s'il a des heures cette période (pas d'exception « historique ») ; un employé à salaire fixe (`isSalaried`) sans heures réelles pointées est traité comme les employés horaires — exclu lui aussi (pas de traitement spécial pour les salariés fixes). **Portée volontairement limitée aux 2 rapports PDF** — la grille écran « Salaires & Pourboires » (`renderSalaires`, saisie des heures) n'est **pas** touchée : elle doit continuer à montrer tout le monde pour permettre la saisie initiale des heures. **Sécurité** : aucun changement de règles Firestore ni de données — uniquement un filtre d'affichage côté client sur des données déjà chargées. Banc d'essai : `node --check` OK (pages-payroll.js). `CACHE_VERSION` → `v3.65.0`.
>
> ⚠️ **14 septembre 2026 — v3.64.0** — **Dépenses : ajout rapide fournisseur/catégorie (+) + case « Sans taxes » + frais supplémentaires**. Trois demandes liées sur le formulaire d'ajout/édition de dépense (`openExpenseModal`, `pages-finance.js`) : (1) **Bouton « + » à côté de Fournisseur et Catégorie** — ouvre une mini-modale (`openQuickSupplier` existait déjà en code mort et est maintenant branché ; `openQuickCat` est nouveau) pour créer l'entrée sans quitter le formulaire. Tout ce qui était déjà tapé (description, montant, notes, etc.) est préservé et restauré au retour (`captureExpenseDraft`), et le nouveau fournisseur/la nouvelle catégorie est présélectionné(e) au retour. (2) **Case « Sans taxes »** (`ex-notax`, champ `noTax` persisté) : désactive et vide TPS/TVQ en un clic au lieu de les mettre à 0 manuellement. (3) **Frais supplémentaires** (ex. livraison) : liste de lignes description+montant ajoutables/supprimables (`expenseExtraRows`, même pattern que la composition de recette en Menu), sommées dans le montant avant taxes et sauvegardées à part dans `customLines[]` pour rester modifiables — le champ `amount` global (déjà utilisé partout pour les totaux/rapports/dashboard) inclut donc toujours tout, sans rien à changer ailleurs. CSS ajouté : `.field-with-add`, `.expense-extras(__header)`, `.expense-extra-row`. Banc d'essai : `node --check` OK (pages-finance.js, i18n.js). `CACHE_VERSION` → `v3.64.0`.
>
> ⚠️ **13 septembre 2026 — v3.63.0** — **Cohérence des couleurs de section (Cuisine/Service/Autre) entre Horaires, Salaires & Pourboires et Simulation + légende visible**. Suite à une revue de cohérence du design demandée par l'utilisateur (repérée via le cas Eileen ci-dessus, v3.62.0) : sur **Salaires & Pourboires**, la couleur de la ligne/carte d'un employé (cellule nom, cartes shift, cellule totaux) était dérivée du **groupe de pourboires effectif** (`getEffectiveTipGroup` : cuisine/service/**exclu**) au lieu de sa **section réelle** (`emp.section`) — un employé Service marqué « Sans pourboire » (`noTips`, ou dérogation « Exclu ») s'affichait donc tout en **gris**, la même couleur que la section « Autre » sur Horaires/Simulation, alors qu'il est bien en Service. **Correctifs** (`pages-payroll.js`) : nouvelle variable `sectionClass` dérivée de `emp.section` (comme sur Horaires, fallback `"service"`), utilisée pour la cellule nom (`.schedule-empgrid-emp`), les cartes shift (`.shift-card--compact`) et la cellule totaux (`.schedule-empgrid-total`) — leur couleur reflète maintenant toujours la vraie section, exactement comme documenté dans le commentaire CSS déjà présent (« La couleur reflète sa section »). Le sélecteur de groupe de pourboires (`.payroll-section-select`, `groupClass`, inchangé) garde lui la couleur "Exclu" — c'est justement sa fonction d'afficher le groupe effectif. Le badge **« Sans pourb. »** (déjà distinct, rouge/rose — `.no-tips-badge`) reste le seul indicateur visuel de l'exclusion des pourboires. **Nouvelle légende couleurs** (`renderSectionLegend()`, `pages-hr.js`, réutilisée par `pages-payroll.js` et `pages-simulations.js`) : 3 pastilles « 🟡 Cuisine · 🔵 Service · ⚪ Autre » affichées juste au-dessus des 3 grilles empgrid (Horaires, Salaires, Simulation) — le code couleur n'était expliqué nulle part dans l'UI avant. CSS : `.section-legend(__item/__dot)`. **Fix mineur** (`sidebar.js`) : le titre de la barre du haut de la page Dépenses disait « Dépenses » alors que le titre H1 de la page dit « Dépenses & Revenus » (deux clés i18n différentes, `nav_expenses` vs `exp_title`) — le topbar utilise maintenant `exp_title` comme le H1 ; le libellé court « Dépenses » reste inchangé dans la sidebar. **Sécurité** : aucun changement de règles Firestore ni de données — uniquement de l'affichage dérivé (CSS/HTML). Banc d'essai : `node --check` OK (pages-hr.js, pages-payroll.js, pages-simulations.js, sidebar.js). `CACHE_VERSION` → `v3.63.0`.
>
> ⚠️ **13 septembre 2026 — v3.62.0** — **Horaires : ordre par défaut Cuisine → Service → Autre + la copie de semaine préserve l'ordre affiché**. Demande utilisateur : (1) en copiant l'horaire vers la semaine suivante (« Copier → S+1 »), l'ordre des employés changeait et devait être refait manuellement ; (2) l'ordre par défaut (nouvelle semaine jamais réordonnée) devrait toujours grouper Cuisine → Service → Autre plutôt qu'un ordre arbitraire ; (3) question sur un employé (Eileen) affiché en gris au lieu du bleu Service dans la grille Horaires. **Correctifs** (`pages-hr.js`) : (1) `duplicateScheduleToNextWeek` calcule désormais l'ordre **affiché** de la semaine source (`visibleScheduleEmployees`) et l'écrit dans `settings/schedule.weekOrder[weekKeySuivante]` au moment de la copie (même batch Firestore que les shifts) — la semaine cible démarre donc avec exactement le même ordre que la source, plus besoin de réordonner. (2) Nouveau helper `employeeSectionPriority(emp)` (Cuisine=0, Service=1, Autre=2) branché en **tie-break** dans `visibleScheduleEmployees` (avant le fallback `sortOrder`) et dans le tri des **cartes équipe** (`renderEmployes`) — s'applique seulement quand aucun ordre explicite (`weekOrder` / drag & drop) n'existe déjà pour cet employé cette semaine ; un ordre déjà personnalisé manuellement n'est jamais perturbé. (3) **Pas un bug de code** : la couleur de la barre latérale/badge est dérivée de `emp.section` (`cuisine` → ambre, `service` → bleu, tout le reste → gris « Autre », voir `secCls`/`accentColor` dans `pages-hr.js`) ; un `section` absent retombe sur `service` (bleu) par défaut — donc un employé gris a nécessairement `section: "other"` réglé explicitement sur sa fiche. Pour repasser Eileen en bleu : fiche employé → champ **Section** → choisir **« Service à la clientèle »** → Enregistrer (pas de correctif de code requis ; pas d'accès direct à Firestore depuis cette session pour le faire à la place de l'utilisateur). **Sécurité** : aucun changement de règles Firestore — écriture sur `settings/schedule.weekOrder` déjà couverte par les règles existantes (admin/chef, comme le drag & drop manuel). Banc d'essai : `node --check` OK (pages-hr.js). `CACHE_VERSION` → `v3.62.0`.
>
> ⚠️ **26 août 2026 — v3.61.0** — **Audit UX : 6 clarifications d'affichage sans nouvelle fonctionnalité**. Suite à un audit multi-angles (qualité de code, UX, performance, sécurité) demandé par l'utilisateur, correction des 6 trouvailles de la section UX/clarté à l'écran. (1) **Ratio salaires/ventes réelles** (`pages-payroll.js`) : la carte utilisait des seuils codés en dur (32 %/40 %) et cachait sa cible dans un tooltip, pendant que la carte « Rentabilité de la semaine » juste en dessous (v3.60.0) utilise la vraie cible configurable `targetRatio` affichée en clair — les deux pouvaient donner des verdicts contradictoires pour la même semaine. Alignée sur `targetRatio` (zone d'alerte = cible + 8 pts, même largeur que l'ancien 32→40), cible affichée en texte visible (plus seulement en tooltip), titre renommé « Ratio salaires / ventes réelles » pour la distinguer de la carte « ventes nettes ». (2) **Profit/Déficit qualifié partout** (`pages-finance.js`, `i18n.js`) : la carte-stat principale (taxes incluses sur les dépenses) et l'aperçu de rapport personnalisé + le PDF exporté (avant taxes, comme l'export Excel déjà correct) affichaient juste « Profit »/« Déficit » sans préciser la base de calcul — deux bases différentes, même mot. Qualificatif ajouté partout : « (taxes incluses) » sur la carte-stat, « (avant taxes) » sur l'aperçu et le PDF (aligné sur l'Excel). Aucun changement de calcul, uniquement de libellé. (3) **Verdict ✓/⚠ dans les exports PNG admin** (`pages-hr.js` `exportScheduleAsPNGAdmin`, `pages-simulations.js` `exportSimAsPNGAdmin`) : le ratio salaires/ventes s'affichait en texte gris plat sans indication bon/mauvais dans les images exportées, contrairement aux cartes à l'écran. Ajout d'un ✓/⚠ coloré + rappel de la cible sur la ligne « ratio réel » (Horaires) ; wording « à ratio X% » → « cible X% » aux deux endroits pour clarifier que c'est la cible réglée, pas une mesure. (4) **Avertissement « pas de données A-1 » rendu visible** (`pages-rapports.js`) : caché dans le tooltip d'une icône ⚠, invisible au toucher sur iPad. Texte ajouté en clair à côté du bouton Vs année précédente. (5) **Badge « Expirée » automatique sur les soumissions** (`pages-quotes.js`) : contrairement aux factures en retard (badge rouge auto), une soumission dont `validUntil` est dépassée ne montrait rien de spécial tant que le statut n'était pas changé manuellement. Nouveau badge visuel auto (même style que `.invoice-overdue-tag`) quand `validUntil` < aujourd'hui et statut ∉ {acceptee, refusee, expiree} — **purement visuel, n'écrit jamais dans `qt.status`**. (6) **Libellé « Exclu » clarifié** (`pages-payroll.js`) : l'option du select de dérogation de section disait juste « Exclu » alors que la fiche employé et la grille disent « Sans pourboire » pour le même concept → renommée « Exclu (sans pourboire) ». **Sécurité** : aucun changement de règles Firestore ni de données — uniquement de l'affichage dérivé et 2 nouvelles classes CSS purement visuelles. CSS : `.reports-yoy-warn-text`, `.quote-expired-tag`, `.quote-card__validity--expired`. Banc d'essai : `node --check` OK (pages-payroll.js, i18n.js, pages-finance.js, pages-hr.js, pages-simulations.js, pages-rapports.js, pages-quotes.js). `CACHE_VERSION` → `v3.61.0`.
>
> ⚠️ **26 août 2026 — v3.60.0** — **Salaires & Pourboires : clarté de la rentabilité (% des ventes nécessaires atteint) + % de pourboire par jour/semaine**. Trois retouches demandées sur les cartes déjà en place depuis v3.59.0, aucun nouveau champ Firestore. (1) **Rentabilité (jour + semaine)** (`pages-payroll.js`) : le gros % affichait le ratio brut coût/ventes (« 33,9 % » — ne dit pas si c'est bon ou mauvais au premier coup d'œil). Remplacé par le **% des ventes nécessaires déjà atteint** : `salesNeeded = coût salarial RÉEL ÷ cible` (toujours basé sur les heures **pointées**, `dayLaborByDk`/`payedWage` — jamais l'horaire planifié), `pctReached = ventes nettes ÷ salesNeeded × 100`. 100 % = cible tout juste atteinte, < 100 % = ventes manquantes, > 100 % = cible dépassée. Affiché « XX,X % atteint » sur la carte semaine et « XX % atteint » sur les 7 chips/jour (nouveau `.payroll-profit-day__pct-label`). Le montant en $ sous le % passe du *budget salarial restant* (`net × cible − salaires`) au **montant de ventes manquant ou en surplus** (`surplus = net − salesNeeded`), affiché « +123,45 $ au-dessus de la cible » ou « 274,47 $ manquant » — cohérent d'échelle avec le %. Tooltip (`title`) enrichi avec le montant de ventes nécessaires. (2) **% Pourboire** (carte « Pourboires & ventes nettes ») : nouveau total semaine `.payroll-tips-total--pct` (pourboires ÷ ventes nettes de la semaine) à côté des 2 totaux existants, + une ligne `.payroll-tips-day__pct` sous les 2 champs de chaque jour (pourboire du jour ÷ ventes nettes du jour) — dérivés de `tipsByDay`/`netByDay` déjà saisis, aucune nouvelle saisie. **Sécurité** : aucun changement de règles Firestore ni de données — uniquement de l'affichage dérivé côté client. CSS : `.payroll-tips-total--pct`, `.payroll-tips-day__pct`, `.payroll-profit-day__pct-label`. Banc d'essai : `node --check` OK (pages-payroll.js). `CACHE_VERSION` → `v3.60.0`.
>
> ⚠️ **10 août 2026 — v3.59.0** — **Salaires & Pourboires : ventes nettes par jour, rentabilité jour/semaine, salaire estimé vs payé**. Trois ajouts liés sur la page **Salaires & Pourboires**, alimentés depuis les données déjà présentes (heures pointées, taux datés, horaire planifié) + une nouvelle saisie manuelle. (1) **Ventes nettes de taxes par jour** (`pages-payroll.js`) : nouveau champ `payroll/{weekId}.netByDay{dk}` saisi directement dans la carte **« Pourboires & ventes nettes »** (renommée), à côté du pourboire du jour — 2 inputs empilés par jour (`updateNetForDay`, même mécanique que `updateTipForDay` : valeur ≤ 0/vide supprime la clé). Total « Ventes nettes » affiché à côté du total pourboires. (2) **Salaires — estimé vs payé** : nouvelle carte comparant le **salaire estimé** (heures **planifiées** dans Employés & Horaires × taux daté effectif) au **salaire payé** (heures **réellement pointées** × taux daté = `sumGross`, déjà affiché ailleurs comme « Salaires bruts »), avec l'écart en $ (vert si sous le planifié, ambre si dépassement ≤ 5 %, rouge au-delà). Pour un salarié, estimé = payé (montant fixe, pas d'écart possible). (3) **Rentabilité jour/semaine — basée sur le ratio cible** : nouvelle carte « Rentabilité de la semaine ». Une journée/semaine est jugée rentable quand son **ratio réel salaires ÷ ventes nettes** reste **sous le ratio cible configuré dans Employés & Horaires** (`scheduleSettings.salesRatio`, ex. 32 % — le même réglage qui sert de cible au planning des horaires), pas via un simple solde net − salaires (qui ignore food cost, loyer, etc.). Affiche le **ratio réel** de la semaine (%) + l'**écart en $ vs le budget cible** (`net × cible − salaires réels` : positif = sous le budget, négatif = dépassement), et un bandeau de **7 chips par jour** (ratio % + écart $, vert/rouge/gris si aucune vente nette saisie) pour repérer d'un coup d'œil les journées hors cible. Coût salarial du jour (`dayLaborByDk`) = heures pointées × taux du jour par employé horaire ; pour un salarié, le montant fixe hebdo est étalé également sur les jours ouverts de la semaine (approximation — un salarié n'a pas de coût quotidien naturel). Le bouton **« Annuler mes saisies »** et sa modale de confirmation effacent désormais aussi `netByDay` (compteur + libellé mis à jour). **Sécurité** : aucun changement de règles Firestore — `netByDay` est un champ de `/payroll/{weekId}`, déjà couvert par la règle existante `allow write: if isAdmin()` (les rôles non-admin restent limités à `actualShifts` via `_payrollPunchUpdateOnly`, `netByDay` ne leur est pas accessible, comme `tipsByDay`). CSS : `.payroll-estimate-card(__grid/__item)`, `.payroll-profit-card(__days/__day)`, `.payroll-tips-total--net`, `.payroll-tips-day__input--net`. Banc d'essai : `node --check` OK (pages-payroll.js, sw.js). `CACHE_VERSION` → `v3.59.0`. — *Précédent :* **v3.58.0** — **Horaires : heures exceptionnelles un jour de congé**. Avant, marquer un employé en congé bloquait totalement la saisie d'un quart ce jour-là (utile pour verrouiller la semaine, mais trop rigide quand l'employé vient quand même travailler 1-2 h pendant ses vacances). (1) **Rendu grille** (`pages-hr.js`, `renderEmployes`) : la carte « Congé » ne s'affiche que si **aucun shift** n'existe ce jour-là (`leave && !hasShift`) ; dès qu'un shift est saisi, la carte shift normale s'affiche avec un petit badge **« Congé »** (`.shift-leave-tag`, couleur du type de congé) en plus des heures/coût. (2) **Modal Congé** (`openTimeOffCellModal`) : nouveau bouton **« Ajouter des heures quand même »** → ouvre `openShiftModal` sans retirer le congé. **Modal Shift** (`openShiftModal`) : bandeau d'info si le jour est déjà en congé, bouton « Marquer en congé » masqué dans ce cas (redondant). (3) **Sauvegarde** (`saveShiftFromModal`) : retrait du blocage dur `isTimeOff` qui refusait l'écriture ; toast informatif distinct si le jour est en congé. (4) **Totaux & exports** : `shifts[dk]` n'est plus forcé à `null` sur un jour de congé dans le calcul des totaux (`renderEmployes`) ni dans les deux `shiftOnDay()` des exports PNG (équipe + admin) — les heures exceptionnelles comptent partout où les heures normales comptent. **Inchangé (volontairement)** : le **glisser-déposer** d'un shift vers un jour de congé reste bloqué (`shiftCardDrop`) pour éviter un déplacement accidentel ; `duplicateScheduleToNextWeek` continue de nettoyer les quarts fantômes sur les jours de congé cibles à la copie de semaine. **Sécurité** : aucun changement de règles Firestore (écriture sur `shifts`/`timeOff` de `/employees`, déjà admin only). CSS : `.shift-leave-tag`. Banc d'essai : `node --check` OK (pages-hr.js, sw.js). `CACHE_VERSION` → `v3.58.0`. — *Précédent :* **v3.57.2** — **Fix : 2ᵉ plage de service (simulation) non sauvegardée / non reflétée**. Bug : la modale sim se re-rendait à partir de `payrollSimulations` **global**, mis à jour par le listener Firestore **après** `persistSim` (asynchrone) → la plage ajoutée n'apparaissait pas et pouvait être **écrasée** à la modification suivante (lecture d'un état périmé). **Fix** : la modale utilise désormais un **brouillon en mémoire** `_simSvcDraft` (comme la modale réelle) ; elle édite/re-rend depuis le brouillon (jamais depuis le global) et persiste en tâche de fond. Signatures simplifiées : `updateSimServiceHours(dow,plageIdx,field,value)`, `addSimServicePlage(dow)`, `removeSimServicePlage(dow,plageIdx)`, + `_renderSimServiceHoursModal`/`closeSimServiceHoursModal`/`_persistSimSvcDraft`. L'éditeur se rafraîchit via le listener (`_editingSimId`). **CSS** : le label de service de l'en-tête de jour (`.schedule-empgrid-day-svc`) passe en `flex-wrap` pour afficher 2 plages sur 2 lignes dans la colonne étroite. `CACHE_VERSION` → `v3.57.2`. — *Précédent :* **v3.57.1** — **Lisibilité modale « Heures de service (simulation) »** : la grille passe à **2 colonnes (jour | plages)** avec un **séparateur horizontal entre chaque jour** et le nom du jour en **accent/gras** aligné en haut, pour voir clairement à quel jour appartiennent les heures (résout un doublon CSS `.sim-svc-row` où l'ancienne règle 4 colonnes écrasait la nouvelle). `CACHE_VERSION` → `v3.57.1`. — *Précédent :* **v3.57.0** — **Heures de service coupées (plusieurs plages par jour) — réel + simulation**. Un jour peut désormais avoir **plusieurs fenêtres de service** (service coupé, ex. **lundi 12:00–14:00 puis 17:00–21:00**) ; seules les heures travaillées **dans une de ces plages** comptent pour les pourboires, le creux (14:00–17:00) est exclu. (1) **Helpers partagés** (`utils.js`) : `normalizeServiceWindows(entry)` (accepte objet unique `{start,end}` **OU** tableau, retourne un tableau trié de plages valides — **rétrocompat**), `intersectShiftWindows(shift, windows)` (somme des intersections sur toutes les plages), `serviceWindowsLabel(entry)` (libellé « 12:00–14:00 · 17:00–21:00 »). (2) **Paie réelle** (`pages-payroll.js`) : nouveau `getServiceWindows(dow)` (pluriel, normalisé) ; les 3 blocs de calcul (écran `renderSalaires`, PDF 1 sem `generatePayrollPDF`, calcul 2 sem `_computePayrollWeekData`) bouclent sur les plages via `intersectShiftWindows` ; en-tête de grille affiche toutes les plages. **Modèle** `settings/payroll.defaultServiceHours[dow]` accepte objet (1 plage) ou tableau (≥2). **Modale « Heures de service »** refondue (`openServiceHoursModal` + brouillon `_svcHoursDraft` + `svcAddPlage`/`svcRemovePlage`/`svcSetPlageTime`/`toggleServiceDay`) : ajout/retrait de plages par jour ; sauvegarde stocke objet si 1 plage, tableau si ≥2 (`saveServiceHours`). (3) **Simulation** (`pages-simulations.js`) : `computeSimScenario`, `simEffectiveOpenDays`, `_simSvcWindowLabel`, snapshot de création et exports PNG gèrent les plages multiples ; modale sim (`openSimServiceHoursModal`) avec `addSimServicePlage`/`removeSimServicePlage`/`updateSimServiceHours(simId,dow,plageIdx,field,value)` + `_persistSimServicePlages`. Les **quarts coupés des employés** fonctionnaient déjà en simulation (v3.45.0, `simDayShifts`) — inchangés. **⚠ Hors périmètre (à faire séparément) :** les quarts coupés dans le **vrai horaire/pointage** (`emp.shifts[dk]` planifié + `actualShifts` pointés + kiosque `pages-punch.js`) restent **un seul `{start,end}` par jour** — chantier à risque réservé à une session dédiée. **Sécurité** : aucun changement de règles Firestore (`settings/payroll` déjà admin only ; `serviceHours` du scénario déjà admin only). CSS : `.payroll-svc-plage-*`, `.sim-svc-plage*`. Banc d'essai : `node --check` OK (utils.js, pages-payroll.js, pages-simulations.js, sw.js) + 13/13 cas (normalisation objet/tableau/tri/filtre, exemple lundi 12–14 + 17–21 = 6h éligibles pour un quart 11–22, 0h dans le creux, à-cheval 1+1, rétrocompat fenêtre unique, formes de stockage 0/1/≥2 plages). `CACHE_VERSION` → `v3.57.0`. — *Précédent :* **v3.56.0** — **Salaires : bonus hebdomadaire par employé**. Nouvelle prime discrétionnaire, **0 $ par défaut**, saisissable par employé pour la semaine courante et **ajoutée au total à payer** (salaire + pourboires + bonus). (1) **Stockage** : `payroll/{weekId}.bonusByEmp{empId}` (admin only ; une valeur ≤ 0/vide **supprime la clé** → retour au défaut 0 $). Helpers `getWeekBonus(empId)` + `updateEmployeeBonus(empId, value)` (bloqué si semaine verrouillée, sur le modèle de `updateTipForDay`). (2) **Écran** (`pages-payroll.js`, `renderSalaires`) : ligne **« Bonus »** avec **champ éditable** (`.payroll-bonus-input`, ambre quand > 0) dans la cellule totaux de chaque employé, entre Pourb et Total ; `row.bonus` intégré à `totalPay` et nouveau `sumBonus`. Panneau totaux semaine : ligne **« Bonus + {montant} »** (affichée seulement si `sumBonus > 0`) entre « Sans pourb. » et « Pourboires », de sorte que **Sans pourb. + Bonus + Pourboires = Avec pourb.** (3) **Rapports PDF** : bonus inclus dans le total à payer des **deux** rapports, plus un **bloc récap « Bonus de la semaine / des 2 semaines »** listant **uniquement les employés avec un bonus > 0**. Dans le PDF 2 semaines (`generateBiWeeklyPDF` / `_computePayrollWeekData`), la fiche « Détail par semaine » gagne une ligne **Bonus** conditionnelle (fiche 28→33 mm) pour garder l'arithmétique juste ; agrégation `bonus1`/`bonus2` + `sumsCombined.bonus`. (4) **Verrouillage** (`_computeWeekGrossWage`) : le bonus est un **coût employeur**, donc il est **ajouté à la dépense « Salaires sem. X »** créée dans Dépenses & Revenus au verrouillage (les pourboires, eux, restent exclus car ils viennent des clients). **Sécurité** : aucun changement de règles Firestore (écriture admin sur `/payroll` déjà autorisée). Banc d'essai : `node --check` OK + 15/15 cas (défaut 0, garde négatif/NaN, invariant des totaux écran, filtrage rapport bonus > 0, agrégation 2 semaines). `CACHE_VERSION` → `v3.56.0`. — *Précédent :* **v3.54.1** — **PDF paie 2 semaines : colonne « Total » des pourboires par employé**. Le tableau récap par employé du **PDF 2 semaines** (`buildTwoWeekPayrollPDF`) avait « Pourb S1 » et « Pourb S2 » mais **pas le total des deux semaines par employé**. Ajout d'une **3ᵉ colonne « Total »** au bloc POURBOIRE = `tips1 + tips2` par employé (vert, gras), et **total combiné** des pourboires dans la ligne TOTAUX (`sumsCombined.tips`). Largeurs ajustées pour rester en landscape Letter (COL_NAME 44→42, COL_SECTION 24→22, COL_SAL/COL_TIP 20→19 ; total tableau **232 mm** ≤ 255,4). Aucun changement de données/règles. Banc d'essai : `node --check` OK + largeur validée. `CACHE_VERSION` → `v3.54.1`. — *Précédent :* **v3.54.0** — **Salaires : montant des pourboires distribués par semaine (triplet avant / pourboires / après)**. Demande : on avait déjà « Sans pourb. » (avant) et « Avec pourb. » (après) mais pas le **montant des pourboires** lui-même de façon visible. (1) **Écran** (`pages-payroll.js`, boîte de totaux du panneau Salaires) : nouvelle ligne **« Pourboires + {montant} »** (vert) insérée **entre** « Sans pourb. » et « Avec pourb. », de sorte que **Sans pourb. + Pourboires = Avec pourb.** Le montant = `sumTips` (= somme des `tipShare` distribués = `sumTotal − sumGross`). CSS `.payroll-totals-final-row--tips` (vert clair/foncé selon thème). (2) **PDF 2 semaines** (`buildTwoWeekPayrollPDF`, fiche « Détail par semaine ») : la fiche passe de 2 à **3 lignes explicites** — *Avant pourb.* / *Pourboires +* (vert) / *Après pourb.* — `ficheH` 23→28 mm, ligne contextuelle (heures · ventes · % des ventes) déplacée en bas. Le **PDF 1 semaine** affichait déjà le KPI « Pourboires distribués », inchangé. **Sécurité** : aucun changement de données ni de règles Firestore (lecture seule des pourboires déjà chargés). Banc d'essai : `node --check` OK + vérif `avant + pourboires = après`. `CACHE_VERSION` → `v3.54.0`. — *Précédent :* **v3.53.0** — **Pointage : fix quart de nuit (passage de minuit) + réinitialisation d'une journée d'employé (Salaires)**. Deux demandes. (1) **Bug quart de nuit** (`pages-punch.js`) : un employé qui entre le soir (ex. 22:00) et **sort le lendemain matin** (ex. 00:56) voyait sa sortie tomber sur un **nouveau jour** (le lendemain n'a pas d'entrée → l'ancien code écrivait `actualShifts[lendemain].end`, une sortie orpheline) tandis que la veille restait avec une entrée sans sortie → heures éparpillées sur 2 jours, calcul faux. **Fix** : si un employé pointe **SORTIE** le matin **sans entrée aujourd'hui** ET qu'un quart est resté **ouvert la veille** (entrée présente, pas de sortie), on **ferme ce quart-là sur la VEILLE** (`_punchCloseOvernightShift`) — `hoursFromShift()` gère déjà le passage de minuit (22:00 → 00:56 = ~2h56 sur le même enregistrement). **Garde-fous** : on n'agit que tôt le matin (`PUNCH_OVERNIGHT_CUTOFF_HOUR = 10`) et si la durée reste plausible (`PUNCH_OVERNIGHT_MAX_HOURS = 12`), pour ne pas fermer par erreur un quart de **jour** oublié la veille. Lecture/écriture **directe** du doc `payroll/{weekId}` de la veille (`_punchFetchShiftForDate` + `_punchMondayOf`) → gère aussi le cas **dim→lun** où la veille est dans une **semaine ISO précédente**. L'écran employé affiche un **indice bleu** (`_punchYesterdayOpenShiftSync`, `punch_overnight_hint` FR/ES) : « Quart de nuit ouvert depuis hier — entrée à {t}. Appuie sur SORTIE pour le fermer. ». CSS `.punch-state-info--overnight`. (2) **Réinitialiser une journée** (`pages-payroll.js`) : dans le modal d'un jour de la page **Salaires & Pourboires**, le bouton d'effacement s'affiche désormais dès qu'il y a **une saisie quelconque** pour ce jour (`hasActualOverride`, et non plus seulement quand entrée **et** sortie sont présentes) — donc on peut nettoyer une **cellule partielle** (ex. une sortie de nuit tombée le mauvais jour). Renommé **« Réinitialiser la journée »** (`resetPayrollDay`) avec **confirmation** (récap entrée/sortie + date) ; efface toute la cellule via `clearActualShift` pour que l'employé **re-pointe** proprement. **Sécurité** : aucun changement de règles Firestore (écritures sur `actualShifts` de `/payroll`, déjà autorisées). Banc d'essai : `node --check` OK (pages-punch.js, pages-payroll.js, i18n.js) + 8/8 cas de décision quart de nuit + cas de passage de semaine validés. `CACHE_VERSION` → `v3.53.0`. — *Précédent :* **v3.52.1** — **Fix congés à la copie de semaine** : un jour de congé sur la semaine cible n'est plus écrasé par un quart copié (quart « fantôme »), et les jours de congé sont désormais exclus partout — totaux d'heures/coût à l'écran et exports **PNG** (équipe + admin) affichent « Congé » au lieu des heures copiées. Robuste aux données déjà corrompues. `CACHE_VERSION` → `v3.52.1`. — *Précédent :* **v3.52.0** — **Taux horaire daté : un changement de salaire s'applique à partir d'une date d'effet choisie (historique de taux)**. Un employé peut maintenant avoir un **historique de taux** : un nouveau taux entré avec une **date d'effet** ne s'applique qu'aux semaines **à partir de cette date** ; les semaines antérieures conservent l'ancien taux, et les calculs de paie/horaire utilisent le bon taux par jour (gère même une hausse en cours de semaine). **Modèle** : `/employeesComp/{empId}` gagne **`rateHistory[]`** = `[{ rate, from:"YYYY-MM-DD" }]` (paliers triés). `hourlyRate` reste écrit mais représente désormais le **taux en vigueur aujourd'hui** (dérivé de l'historique par `_applyEmployeeComp`, qui propage aussi `rateHistory` sur l'objet emp en mémoire). **Helpers** (`pages-hr.js`) : **`effectiveHourlyRate(emp, dateKey)`** (taux du palier le plus récent dont `from` ≤ date ; comparaison de chaînes `YYYY-MM-DD`), `normalizeRateHistory` (tri + fusion des doublons de date + suppression des paliers redondants), `todayKey`, `fmtDateLong`. **Éditeur employé** : à côté du taux, nouveau champ **« En vigueur à partir du »** (défaut = aujourd'hui) ; à la sauvegarde, `saveEmployee` **scelle l'ancien taux dans le passé** (`2000-01-01`) la 1re fois puis **upsert** le taux saisi à sa date d'effet. Un bloc **« Historique des taux »** (lecture seule, paliers à venir en bleu) avec ✕ pour retirer un palier (`removeRateHistoryEntry`). **Application du taux daté** (portée paie **+** horaires, comme demandé) : `renderSalaires` (calcul principal), `_computeWeekGrossWage` (verrouillage de semaine → dépense Salaires), les **2 builders PDF** de paie (1 sem + 2 sem), l'**horaire** (`renderEmployes`) et le **PNG admin** des horaires. Pour les salariés (montant fixe), le taux est celui du **début de semaine** ; pour les horaires, le coût est sommé **jour par jour** au taux effectif de chaque jour. Les vues sans contexte de semaine (team cards, Simulation, Dashboard, extras) utilisent `emp.hourlyRate` = taux courant. **Sécurité** : aucun changement de règles (`rateHistory` est un champ de `/employeesComp`, déjà admin only) — **pas besoin de republier `firestore.rules`**. CSS : `.rate-hist*`. Banc d'essai : `node --check` OK (pages-hr.js, pages-payroll.js, state.js) + scénario validé (sem 1 à 17 $, sem 2 à 18 $ → 510 $ / 540 $ ; hausse en milieu de semaine au prorata jour par jour). `CACHE_VERSION` → `v3.52.0`.
>
...
- **Collections Firestore** :
  ...
  - `payroll` — paie hebdomadaire (un doc par semaine ISO `YYYY-Www`) :
    - `weekId`, `weekStart`, `totalTips` (legacy), `serviceHours` `{dk: {start,end}}`, `actualShifts` `{empId: {dk: {start,end}}}`, `notes`, `createdAt`/`updatedAt`
    - **`tipsByDay{dk}`** — pourboires reçus par jour (remplace `totalTips`, dont la somme sert de total semaine, rétrocompat si vide). **`netByDay{dk}`** (v3.59.0) — ventes nettes de taxes saisies par jour, base du calcul de rentabilité (carte « Rentabilité de la semaine »). **`bonusByEmp{empId}`** — prime hebdo discrétionnaire par employé (v3.56.0). Une valeur ≤ 0/vide supprime la clé (retour au défaut) pour les trois champs.
...
**Changements** (`pages-hr.js`)
- La carte « Congé » ne s'affiche que si **aucun shift** n'existe ce jour-là. Dès qu'un shift est saisi, la carte shift normale (heures + coût) s'affiche avec un petit badge **« Congé »** (`.shift-leave-tag`) — le congé n'est jamais retiré automatiquement.
- Modal Congé (`openTimeOffCellModal`) : nouveau bouton **« Ajouter des heures quand même »** vers `openShiftModal`. Modal Shift : bandeau d'info si déjà en congé, bouton « Marquer en congé » masqué dans ce cas.
- `saveShiftFromModal` : retrait du blocage `isTimeOff` qui empêchait l'écriture du shift.
- Totaux (`renderEmployes`) + exports PNG équipe/admin (`shiftOnDay`) : ne forcent plus `null`/0 h sur un jour de congé — les heures exceptionnelles comptent partout.

**Inchangé** : glisser-déposer vers un jour de congé reste bloqué (évite un déplacement accidentel) ; `duplicateScheduleToNextWeek` continue de nettoyer les quarts fantômes à la copie de semaine. Aucun changement de règles Firestore. `CACHE_VERSION` → `v3.58.0`.
